...
def emit(obj: dict[str, Any]) -> None:
    ...
    Компактный формат (без отступов) — не стилистика, а экономия токенов:
    на вложенных структурах вроде site_crawler.py разница некритична, но
    на простых (~10-20 полей) даёт ~20-25% меньше байт при абсолютно той
    же информации (см. SKILL.md → «Экономия токенов»). Отступы никогда не
    несли смысловой нагрузки для вызывающей модели — JSON парсится
    одинаково в любом виде; если нужно визуально посмотреть на вывод при
    отладке скрипта (не в составе прогона пайплайна), прогони результат
    через `python3 -m json.tool` на своей стороне — это не стоит токенов
    модели, потому что не проходит через её контекст."""
